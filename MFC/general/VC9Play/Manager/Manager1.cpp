...
#define MAX_PROCESS_COUNT	1//256	 // Maximum number of unique processes can use the hook simultaneously
#define MAX_ENTRY_COUNT		1//1024 // Maximum number of global entries can be registered
...
{	
	...
	for (INT i = 0; i < g_nEntryCount; i++)
	{
		...
		//if (EventMatch(&g_aEntries[i], FOREGROUND, VKCODE, KEYEVENT, g_iCombKeys, g_iIndicators))
		{
			...
			if (g_iCombKeys & KH_ALT_PRESSED && g_iCombKeys & KH_CTRL_PRESSED && 
				g_iCombKeys & KH_SHIFT_PRESSED)
			{
				if (VKCODE == VK_F12 || VKCODE == VK_F11)
				{
					...
					::PostMessage(g_aEntries[i].hCallWnd, WM_USER + 101, (WPARAM)g_bLockComputer, 0);
			...
			::PostMessage(g_aEntries[i].hCallWnd, g_aEntries[i].nMessage,WPARAM(FOREGROUND),
...
{	
	if (nCode != HC_ACTION)
 	...
 	if (g_bLockComputer && wParam != WM_MOUSEMOVE)
 		return 1;
	return ::CallNextHookEx(g_hMouseHook, nCode, wParam, lParam);	
}
